...
import { IntegratorAppModule } from "./app.module";
...
async function bootstrap() {
  ...
  try {
    ...
    logger.info(`ℹCreando instancia del módulo IntegratorAppModule...`);
    const app = await NestFactory.create(IntegratorAppModule, {
    ...
    const swaggerPath = setupSwagger(
      app,
      "api-docs",
      "Integrator Service API",
      "API completa para gestión de Integrators con documentación automática",
      "1.0"
    );
