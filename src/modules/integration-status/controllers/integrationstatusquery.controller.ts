...
import { IntegrationStatusDto } from "../dtos/all-dto";
...
@ApiTags("IntegrationStatus Query")
@UseGuards(IntegrationStatusAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("integrationstatuss/query")
export class IntegrationStatusQueryController {
  #logger = new Logger(IntegrationStatusQueryController.name);

  constructor(private readonly service: IntegrationStatusQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all integrationstatus with optional pagination" })
  @ApiResponse({ status: 200, type: IntegrationStatussResponse })
  @ApiQuery({ name: "options", required: false, type: IntegrationStatusDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<IntegrationStatus>    
  ): Promise<IntegrationStatussResponse<IntegrationStatus>> {
    try {
     
      const integrationstatuss = await this.service.findAll(options);
      logger.info("Retrieving all integrationstatus");
  ...
  @ApiOperation({ summary: "Find integrationstatus by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter integrationstatus', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: IntegrationStatussResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "IntegrationStatus no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find integrationstatuss with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<IntegrationStatus>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all integrationstatuss" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count integrationstatuss with conditions" })
  @ApiResponse({ status: 200, type: IntegrationStatussResponse<IntegrationStatus> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades IntegrationStatuss no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one integrationstatus with conditions" })
  ...
  @ApiOperation({ summary: "Find one integrationstatus or return error" })
  @ApiResponse({ status: 200, type: IntegrationStatusResponse<IntegrationStatus> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad IntegrationStatus no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get integrationstatus by ID" })
  @ApiResponse({ status: 200, type: IntegrationStatusResponse<IntegrationStatus> })
  @ApiResponse({ status: 404, description: "IntegrationStatus not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the integrationstatus to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<IntegrationStatusResponse<IntegrationStatus>> {
    try {
      const integrationstatus = await this.service.findOne({ where: { id } });
      if (!integrationstatus) {
        throw new NotFoundException(
          "IntegrationStatus no encontrado para el id solicitado"
        );
      }
