...
import { IntegrationEndpointDto } from "../dtos/all-dto";
...
@ApiTags("IntegrationEndpoint Query")
@UseGuards(IntegrationEndpointAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("integrationendpoints/query")
export class IntegrationEndpointQueryController {
  #logger = new Logger(IntegrationEndpointQueryController.name);

  constructor(private readonly service: IntegrationEndpointQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all integrationendpoint with optional pagination" })
  @ApiResponse({ status: 200, type: IntegrationEndpointsResponse })
  @ApiQuery({ name: "options", required: false, type: IntegrationEndpointDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<IntegrationEndpoint>    
  ): Promise<IntegrationEndpointsResponse<IntegrationEndpoint>> {
    try {
     
      const integrationendpoints = await this.service.findAll(options);
      logger.info("Retrieving all integrationendpoint");
  ...
  @ApiOperation({ summary: "Find integrationendpoint by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter integrationendpoint', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: IntegrationEndpointsResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "IntegrationEndpoint no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find integrationendpoints with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<IntegrationEndpoint>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all integrationendpoints" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count integrationendpoints with conditions" })
  @ApiResponse({ status: 200, type: IntegrationEndpointsResponse<IntegrationEndpoint> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades IntegrationEndpoints no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one integrationendpoint with conditions" })
  ...
  @ApiOperation({ summary: "Find one integrationendpoint or return error" })
  @ApiResponse({ status: 200, type: IntegrationEndpointResponse<IntegrationEndpoint> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad IntegrationEndpoint no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get integrationendpoint by ID" })
  @ApiResponse({ status: 200, type: IntegrationEndpointResponse<IntegrationEndpoint> })
  @ApiResponse({ status: 404, description: "IntegrationEndpoint not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the integrationendpoint to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<IntegrationEndpointResponse<IntegrationEndpoint>> {
    try {
      const integrationendpoint = await this.service.findOne({ where: { id } });
      if (!integrationendpoint) {
        throw new NotFoundException(
          "IntegrationEndpoint no encontrado para el id solicitado"
        );
      }
