...
export class IntegrationEndpointService {
  constructor(private readonly repository: IntegrationEndpointRepository) {}

  // Métodos delegados
  async findAll(options?: any): Promise<IntegrationEndpoint[]> {
  ...
  async findByField(field: string, value: any, page: number, limit: number): Promise<IntegrationEndpoint[]> {
    return this.repository.findByField(field, value, page, limit);
  }

  async findWithPagination(options: any, page: number, limit: number): Promise<IntegrationEndpoint[]> {
  ...
  async findOne(where?: Record<string, any>): Promise<IntegrationEndpoint | null> {
  ...
  async update(id: string, partialEntity: Partial<IntegrationEndpoint>): Promise<IntegrationEndpoint | null> {
