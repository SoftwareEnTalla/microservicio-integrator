...
@Injectable()
export class IntegrationAuthSchemeService {
  constructor(private readonly repository: IntegrationAuthSchemeRepository) {}

  // Métodos delegados
  async findAll(options?: any): Promise<IntegrationAuthScheme[]> {
  ...
  async findByField(field: string, value: any, page: number, limit: number): Promise<IntegrationAuthScheme[]> {
    return this.repository.findByField(field, value, page, limit);
  }

  async findWithPagination(options: any, page: number, limit: number): Promise<IntegrationAuthScheme[]> {
  ...
  async findOne(where?: Record<string, any>): Promise<IntegrationAuthScheme | null> {
  ...
  async update(id: string, partialEntity: Partial<IntegrationAuthScheme>): Promise<IntegrationAuthScheme | null> {
