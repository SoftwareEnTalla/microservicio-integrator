...
@ApiTags("IntegrationAuthScheme Query")
@UseGuards(IntegrationAuthSchemeAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("integrationauthschemes/query")
export class IntegrationAuthSchemeQueryController {
  #logger = new Logger(IntegrationAuthSchemeQueryController.name);

  constructor(private readonly service: IntegrationAuthSchemeQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all integrationauthscheme with optional pagination" })
  @ApiResponse({ status: 200, type: IntegrationAuthSchemesResponse })
  @ApiQuery({ name: "options", required: false, type: IntegrationAuthSchemeDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<IntegrationAuthScheme>    
  ): Promise<IntegrationAuthSchemesResponse<IntegrationAuthScheme>> {
    try {
     
      const integrationauthschemes = await this.service.findAll(options);
      logger.info("Retrieving all integrationauthscheme");
  ...
  @ApiOperation({ summary: "Find integrationauthscheme by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter integrationauthscheme', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: IntegrationAuthSchemesResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "IntegrationAuthScheme no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find integrationauthschemes with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<IntegrationAuthScheme>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all integrationauthschemes" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count integrationauthschemes with conditions" })
  @ApiResponse({ status: 200, type: IntegrationAuthSchemesResponse<IntegrationAuthScheme> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades IntegrationAuthSchemes no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one integrationauthscheme with conditions" })
  ...
  @ApiOperation({ summary: "Find one integrationauthscheme or return error" })
  @ApiResponse({ status: 200, type: IntegrationAuthSchemeResponse<IntegrationAuthScheme> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad IntegrationAuthScheme no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get integrationauthscheme by ID" })
  @ApiResponse({ status: 200, type: IntegrationAuthSchemeResponse<IntegrationAuthScheme> })
  @ApiResponse({ status: 404, description: "IntegrationAuthScheme not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the integrationauthscheme to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<IntegrationAuthSchemeResponse<IntegrationAuthScheme>> {
    try {
      const integrationauthscheme = await this.service.findOne({ where: { id } });
      if (!integrationauthscheme) {
        throw new NotFoundException(
          "IntegrationAuthScheme no encontrado para el id solicitado"
        );
      }
      return integrationauthscheme;
    } catch (error) {
      logger.error(error);
      return Helper.throwCachedError(error);
    }
