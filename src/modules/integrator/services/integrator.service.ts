...
export class IntegratorService {
  constructor(private readonly repository: IntegratorRepository) {}

  // Métodos delegados
  async findAll(options?: any): Promise<Integrator[]> {
  ...
  async findByField(field: string, value: any, page: number, limit: number): Promise<Integrator[]> {
    return this.repository.findByField(field, value, page, limit);
  }

  async findWithPagination(options: any, page: number, limit: number): Promise<Integrator[]> {
  ...
  async findOne(where?: Record<string, any>): Promise<Integrator | null> {
  ...
  async update(id: string, partialEntity: Partial<Integrator>): Promise<Integrator | null> {
