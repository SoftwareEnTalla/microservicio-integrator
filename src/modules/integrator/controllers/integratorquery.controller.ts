...
import { IntegratorDto } from "../dtos/all-dto";
...
@ApiTags("Integrator Query")
@UseGuards(IntegratorAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("integrators/query")
export class IntegratorQueryController {
  #logger = new Logger(IntegratorQueryController.name);

  constructor(private readonly service: IntegratorQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all integrator with optional pagination" })
  @ApiResponse({ status: 200, type: IntegratorsResponse })
  @ApiQuery({ name: "options", required: false, type: IntegratorDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<Integrator>    
  ): Promise<IntegratorsResponse<Integrator>> {
    try {
     
      const integrators = await this.service.findAll(options);
      logger.info("Retrieving all integrator");
      return integrators;
    } catch (error) {
      logger.error(error);
  ...
  @ApiOperation({ summary: "Find integrator by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter integrator', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: IntegratorsResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Integrator no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find integrators with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<Integrator>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all integrators" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count integrators with conditions" })
  @ApiResponse({ status: 200, type: IntegratorsResponse<Integrator> })
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one integrator with conditions" })
  ...
  @ApiOperation({ summary: "Find one integrator or return error" })
  @ApiResponse({ status: 200, type: IntegratorResponse<Integrator> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get integrator by ID" })
  @ApiResponse({ status: 200, type: IntegratorResponse<Integrator> })
  @ApiResponse({ status: 404, description: "Integrator not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the integrator to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<IntegratorResponse<Integrator>> {
    try {
      const integrator = await this.service.findOne({ where: { id } });
      if (!integrator) {
        throw new NotFoundException(
