...
@ApiTags("IntegrationProvider Query")
@UseGuards(IntegrationProviderAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("integrationproviders/query")
export class IntegrationProviderQueryController {
  #logger = new Logger(IntegrationProviderQueryController.name);

  constructor(private readonly service: IntegrationProviderQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all integrationprovider with optional pagination" })
  @ApiResponse({ status: 200, type: IntegrationProvidersResponse })
  @ApiQuery({ name: "options", required: false, type: IntegrationProviderDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<IntegrationProvider>    
  ): Promise<IntegrationProvidersResponse<IntegrationProvider>> {
    try {
     
      const integrationproviders = await this.service.findAll(options);
      logger.info("Retrieving all integrationprovider");
  ...
  @ApiOperation({ summary: "Find integrationprovider by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter integrationprovider', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: IntegrationProvidersResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "IntegrationProvider no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find integrationproviders with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<IntegrationProvider>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all integrationproviders" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count integrationproviders with conditions" })
  @ApiResponse({ status: 200, type: IntegrationProvidersResponse<IntegrationProvider> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades IntegrationProviders no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one integrationprovider with conditions" })
  ...
  @ApiOperation({ summary: "Find one integrationprovider or return error" })
  @ApiResponse({ status: 200, type: IntegrationProviderResponse<IntegrationProvider> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad IntegrationProvider no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get integrationprovider by ID" })
  @ApiResponse({ status: 200, type: IntegrationProviderResponse<IntegrationProvider> })
  @ApiResponse({ status: 404, description: "IntegrationProvider not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the integrationprovider to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<IntegrationProviderResponse<IntegrationProvider>> {
    try {
      const integrationprovider = await this.service.findOne({ where: { id } });
      if (!integrationprovider) {
        throw new NotFoundException(
          "IntegrationProvider no encontrado para el id solicitado"
        );
      }
