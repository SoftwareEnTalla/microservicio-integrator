...
export class IntegrationProviderService {
  constructor(private readonly repository: IntegrationProviderRepository) {}

  // Métodos delegados
  async findAll(options?: any): Promise<IntegrationProvider[]> {
  ...
  async findByField(field: string, value: any, page: number, limit: number): Promise<IntegrationProvider[]> {
    return this.repository.findByField(field, value, page, limit);
  }

  async findWithPagination(options: any, page: number, limit: number): Promise<IntegrationProvider[]> {
  ...
  async findOne(where?: Record<string, any>): Promise<IntegrationProvider | null> {
  ...
  async update(id: string, partialEntity: Partial<IntegrationProvider>): Promise<IntegrationProvider | null> {
