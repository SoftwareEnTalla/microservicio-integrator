...
export const AppDataSource = new DataSource({
  ...
  database: process.env.DB_NAME || "integrator-service",
...
async function checkPostgreSQLExtensions() {
  ...
  const integrator = await pool.connect();

  try {
    for (const ext of REQUIRED_EXTENSIONS) {
      const res = await integrator.query(
      ...
      if (res.rows.length === 0) {
        ...
        await integrator.query('CREATE EXTENSION IF NOT EXISTS "' + ext + '"');
      }
    }
  } finally {
    await integrator.release();
    await pool.end();
  }
}
